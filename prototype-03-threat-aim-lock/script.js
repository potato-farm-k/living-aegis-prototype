const canvas = document.querySelector("#aim-canvas");
...
function resolveThreat(width, height) {
  ...
  const lockReady = visualContact && aim.distance <= settings.aimGuideRadius;
...
function updateSettings(threat) {
  ...
  settingEls.aimDistance.textContent = `${Math.round(threat.aimDistance)} px`;
...
function drawAimGuide(width, height, threat, timestamp) {
  ...
  ctx.lineWidth = threat.lockReady ? 2.5 : 1.5;
