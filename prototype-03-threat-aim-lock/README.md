...
- Aim Guide Radius: 중앙 조준 기준점 기준 `46px`
...
- `Aim Searching`: 위협이 `Visual Contact` 상태지만 중앙 조준 기준점 근처에는 아직 들어오지 않았다.
- `Lock Ready`: `Visual Contact` 상태의 위협이 중앙 조준 기준점의 가이드 반경 안에 들어왔다.
...
- `Visual Contact` 상태에서만 `Lock Ready`가 되는 단순 거리 판정
