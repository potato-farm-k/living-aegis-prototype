...
function getSourceColor(mode) {
  return sourceModes[mode]?.color || "255, 207, 112";
}

function getSourceTextColor(mode) {
  return sourceModes[mode]?.textColor || "rgba(255, 232, 176, 0.94)";
}
...
function drawOrbitalGuide(width, height, timestamp) {
  ...
  const radius = earth.radius * 1.34;

  ctx.save();
  ctx.translate(earth.screenX, earth.screenY);
  ctx.rotate(-0.18);
  ctx.setLineDash([10, 12]);
  ctx.strokeStyle = `rgba(116, 221, 255, ${0.2 + pulse * 0.1})`;
  ctx.lineWidth = 1.6;
  ctx.beginPath();
...
function drawTrajectoryTrace(width, height) {
  ...
  const pointCount = 32;
  const traceColor = getSourceColor(state.sourceMode);

  ctx.save();
  ctx.lineWidth = 1.5;
  ctx.strokeStyle = `rgba(${traceColor}, 0.38)`;
  ctx.beginPath();
  ...
  ctx.setLineDash([5, 9]);
  ctx.strokeStyle = `rgba(${traceColor}, 0.18)`;
  ctx.beginPath();
...
function render(timestamp) {
  ...
  drawOriginGuide(threat, width, height, timestamp);
