...
모든 조합은 하나의 공통 cubic bezier 기반 trajectory model을 사용한다. 모든 위협은 `Lunar Defense Zone` 도달 전에 하늘 영역에서 명확히 드러나는 `visible approach window`를 가지며, 플레이어는 이 구간에서 위협을 보고 조준해 `Lock Ready`를 만들 수 있다.
...
visibleApproachPoint = Lunar Defense Zone 위쪽 하늘 영역
end = Lunar Defense Zone surface anchor
trajectory = source position → radial outward boost → transfer curve
             → visible approach point → terminal descent → Lunar Defense Zone
```

공통 generator는 `sourcePosition - earthCenter`를 정규화한 radial outward 방향에 짧은 boost point를 둔다. 이 boost point를 첫 cubic control point로 사용하므로, 위협은 발사 직후 화면 위쪽으로 고정 이동하지 않고 현재 source에서 지구 중심 반대 방향으로 솟는다. 이후 transfer curve를 지나 동적 달 지평선보다 충분히 위쪽에 있는 visible approach point로 이동하고, 일정 시간 하늘 영역에 머문 뒤 terminal descent를 시작한다.
...
`visible approach window`는 실제 물리 궤도가 아니라 탐색, 조준, `Lock Ready`, 요격 루프를 성립시키기 위한 gameplay readability 규칙이다. 플레이어가 시선을 최대한 위로 올려 지구가 보이지 않는 상태에서도 네 source 조합 모두 terminal descent 전에 포착 가능한 하늘 구간을 지나도록 한다. `Impact Warning`은 Visual Contact가 한 번 이상 발생한 뒤 terminal approach 후반부에만 표시한다. 위협이 계속 달 표면에 가려져 있으면 `Detected / Occluded`를 유지하며 `Lock Ready`를 허용하지 않는다.
...
- Lunar Defense Zone 도달 전에 하늘 영역에서 조준 시간을 보장하는 visible approach window
...
- 네 조합 모두 terminal descent 전에 하늘 영역에서 충분한 Visual Contact 구간을 가지는가?
- 시선을 최대한 위로 올려도 visible approach window에서 위협을 찾고 조준할 수 있는가?
