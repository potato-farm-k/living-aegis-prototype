...
trajectory = common generated path from start through boost direction to end
```

공통 generator는 `sourcePosition - earthCenter`를 정규화한 radial outward 방향에 짧은 boost point를 둔다. 이 boost point를 첫 cubic control point로 사용하므로, 위협은 발사 직후 화면 위쪽으로 고정 이동하지 않고 현재 source에서 지구 중심 반대 방향으로 솟는다. 이후 기존 control point와 고정된 Lunar Defense Zone end를 따라 매끄럽게 방향을 전환한다.
