...
`Impact Warning`은 Lunar Defense Zone surface anchor 도달 직전의 마지막 `2.0초` 동안 반드시 유지한다. 이 phase에서는 `Surface Occluded`가 상태를 덮어쓰지 않으며, 플레이어가 다른 방향을 보면 `Impact Warning + Off-screen`, 올바른 방향을 보면 `Impact Warning + Visual Contact`가 된다. crosshair 근처에서는 `Lock Ready`가 되고 발사 입력으로 요격할 수 있다. 경고가 끝나는 시점이 trajectory의 실제 끝이므로 그때 `Threat Passed`로 전환한다.
...
이번 hotfix는 trajectory 좌표를 바꾸지 않고 phase 시작 시점과 visibility / lock 상태 우선순위만 복구한다. 기존 cubic trajectory, radial outward boost, High / Low 공통 generator, Lunar Defense Zone surface anchor는 그대로 유지한다.
...
- `Impact Warning`: Lunar Defense Zone 도달 전 마지막 `2.0초` 방어 기회다. 이 phase에서는 Surface Occluded가 Lock Ready를 막지 않는다.
- `Threat Passed`: 위협이 trajectory 끝의 Lunar Defense Zone surface anchor에 도달했음을 알리는 간단한 상태이다. horizon 접촉만으로는 이 상태가 되지 않으며, 현재는 `Impact Warning` 종료와 trajectory 끝이 같은 시점이다.
