...
모든 조합은 하나의 공통 cubic bezier 기반 world-space trajectory model을 사용한다. 위협은 화면의 고정된 하늘 영역을 목표로 이동하지 않으며, source position에서 출발해 `Lunar Defense Zone`을 향하는 정해진 경로를 따른다.

```text
start = selected source position
boostDirection = normalize(source position - earth center)
boostPoint = start + boostDirection × boost distance
terminalEntryPoint = lunarDefenseZone + terminalEntryOffset
end = Lunar Defense Zone surface anchor
trajectory = source position → radial outward boost → transfer curve
             → terminal entry point → terminal approach → Lunar Defense Zone
```

공통 generator는 `sourcePosition - earthCenter`를 정규화한 radial outward 방향에 짧은 boost point를 둔다. 이 boost point를 첫 cubic control point로 사용하므로, 위협은 발사 직후 화면 위쪽으로 고정 이동하지 않고 현재 source에서 지구 중심 반대 방향으로 솟는다. 이후 transfer curve와 `Lunar Defense Zone` 위쪽/전방의 world-space `terminal entry point`를 거쳐 terminal approach를 시작한다.
...
`visible approach window`는 screen-space 목표점이 아니다. world-space 궤도의 terminal entry 구간이 현재 플레이어의 하늘/정면 영역에 들어올 때 시야에 드러나는 결과로 본다. 시야 이동은 궤적 좌표를 다시 계산하지 않고 위협이 화면에 보이거나 달 표면에 가려지는 방식만 바꾼다. 즉, `view offset changes what the player sees, not where the threat flies.`

`Lunar Defense Zone`은 정면 보기의 달 표면 영역에서 surface depth 약 `60%`인 위치를 기준으로 world anchor를 정하고, 시야 이동 중에는 이 world anchor 자체를 다시 계산하지 않는다. `terminal entry point`도 이 anchor에 공통 world-space offset을 더해 계산한다.
...
- Lunar Defense Zone Surface Depth: 정면 보기의 달 표면 영역 안에서 약 `60%`인 고정 world anchor
...
- source position에서 지구 중심 반대 방향으로 향하는 공통 radial outward launch boost
- Lunar Defense Zone 기준 world-space terminal entry 구간
- terminal entry 구간이 현재 시야에 들어올 때 결과로 발생하는 visible approach window
...
- terminal entry point가 화면이나 지평선이 아니라 Lunar Defense Zone 기준 world-space 경유점으로 읽히는가?
- 시야를 이동해도 위협 경로는 유지되고 보임/가림 상태만 달라지는가?
- terminal entry 구간이 현재 하늘/정면 영역에 들어오면 Visual Contact와 조준 기회가 생기는가?
...
- 위협 크기, 고도감, 속도 변화, 착탄 예상점과 그림자를 통한 마지막 접근 깊이감 개선
