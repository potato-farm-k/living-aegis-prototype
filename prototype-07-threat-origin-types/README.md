...
이번 단계의 핵심은 실제 궤도 물리나 완성형 전투 루프가 아니라, `Earth Surface Source`, `Orbital Source`, `Behind Surface Approach`가 화면에서 서로 다르게 읽히는지 보는 것이다.
...
- `7`: Earth Scale 7x
- `N`: 새 위협 생성 / Restart
- 화면 버튼: Earth Scale 전환, 기본 정면 보기 복귀, 새 위협 생성 / Restart, Origin Type 선택
...
### Behind Surface Approach

달 표면 아래쪽 또는 달 뒤편에서 접근하는 위협 감각을 확인한다. 초기에는 `Detected / Occluded` 또는 `Impact Warning`처럼 읽히며, 플레이어가 시선을 아래로 향해 달 표면 비중이 커지면 `Visual Contact`가 가능하도록 단순 처리한다. 실제 달 뒤편 물리 계산은 하지 않는다.

## 위협 상태 구분

- `Off-screen`: 위협이 화면 밖에 있으며 edge indicator로 방향만 안내한다.
- `Detected / Occluded`: 위협이 화면 안쪽 방향에 들어왔지만 현재 표시되는 달 표면 영역에 가려져 실제로 보이지 않는 상태로 본다.
...
- `Impact Warning`: 위협 접근 진행도가 높거나 Behind Surface 접근 초기 경고가 필요한 상태 메시지이다.
...
- Behind Surface Approach용 달 표면 뒤 신호 cue
...
점수, 체력, 게임 오버, 웨이브, 난이도 상승, 여러 위협 동시 출현, 사운드, 실제 총알/요격체 궤적, 실제 탄도 계산, 실제 중력 계산, 실제 공격 위성 궤도 계산, 실제 달 뒤편 물리 계산, 지구 표면의 정확한 발사 좌표 계산은 이번 범위에서 제외합니다.
