...
`Visual Contact`는 위협을 고정 하늘 지점으로 보내서 만드는 것이 아니다. 단일 world-space 곡선 위의 위협이 현재 화면의 보이는 하늘 영역에 들어오면 `Visual Contact`로 판정하고, 달 표면에 가려져 있으면 `Detected / Occluded`로 판정한다. 시야 이동은 궤적 좌표를 다시 계산하지 않고 위협이 보이거나 가려지는 방식만 바꾼다.

`Lunar Defense Zone`은 정면 보기의 달 표면 영역에서 surface depth 약 `60%`인 위치를 기준으로 world anchor를 정하고, 시야 이동 중에는 이 world anchor 자체를 다시 계산하지 않는다.
