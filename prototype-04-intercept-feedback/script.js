const canvas = document.querySelector("#intercept-canvas");
...
function resolveThreat(width, height) {
  ...
  const active = !state.intercepted;
  const onScreen = active && isInsideViewport(screenX, screenY, width, height);
  const lunarSurfaceTop = getLunarSurfaceTop(height);
  const occluded = active && onScreen && screenY >= lunarSurfaceTop;
  const visualContact = active && onScreen && !occluded;
  const aim = getAimMetrics(screenX, screenY, width, height);
...
function nextThreat() {
  if (threatPatterns.length < 2) {
    return;
  }

  state.threatIndex = (state.threatIndex + 1) % threatPatterns.length;
...
function updateSettings(threat) {
  ...
  settingEls.aimStatus.textContent = getAimStatusLabel(threat);
  ...
  settingEls.aimDistance.textContent = threat.intercepted ? "-" : `${Math.round(threat.aimDistance)} px`;
...
function isFireWarning(status) {
  return status === "Not Aligned" || status === "Occluded" || status === "No Visual Contact" || status === "Already Intercepted";
}
...
document.querySelector("#next-threat").addEventListener("click", nextThreat);
