# Prototype 01 - Lunar View Control

상태: `1차 구현`

이 프로토타입은 전투 시스템이 아니라, 달 표면에서 지구를 바라보는 기본 구도와 제한된 시야 이동 감각을 확인하기 위한 실험입니다.

## 목적

`living-aegis-simulator`의 `lunar-view-framing-simulator`에서 정리한 1차 달-지구 화면 구도 후보값을 실제 플레이 감각에 가까운 정적 prototype 화면에 적용한다.

시뮬레이터는 값을 찾는 도구이고, 이 prototype은 그 값이 화면에서 게임처럼 느껴지는지 확인하는 실험이다.

## 적용한 기준값

- Earth Scale: `6x` 기본값
- Alternative Earth Scale: `5x`, `7x`
- Earth Vertical Position: `30%`
- Lunar Surface Area: `30%`
- View Movement: 확대된 범위 안에서 제한된 좌우/상하 이동

이 값들은 최종 게임 기준으로 확정된 값이 아니라, 1차 후보를 검토하기 위한 기본값이다.

## 1차 검토 메모

- 현재 기본 Earth Scale 후보는 `6x`로 둔다.
- `5x`와 `7x`는 비교 후보로 유지한다.
- `6x`는 지구 자체의 긴장감과 향후 지구 궤도 공격 위성 표시 공간 사이의 균형을 보기 위한 기준값이다.
- 현재 시야 이동 폭은 1차 검토 기준으로 대체로 적당해 보이며, 이번 단계에서는 추가로 변경하지 않는다.
- 키보드 이동 속도만으로는 체감 판단이 다소 느릴 수 있으므로, 마우스 기반 시야 이동 확인 기능을 추가해 별도로 본다.
- 위쪽 시선 이동 시 지구와 달 표면이 살짝 겹치는 구도는 후속 검토 후보로 둔다.
- 지구-달 겹침 구도는 공격 위성 궤도선 또는 달 표면 이미지가 추가된 뒤 다시 확인한다.

## 2차 검토 메모

- Earth Scale `6x` 기본값에서 화면이 안정적으로 보였다.
- `6x`는 지구의 긴장감과 향후 지구 궤도 공격 위성 표시 공간 사이의 균형을 위한 1차 기준값으로 유지한다.
- `5x`와 `7x`는 비교 후보로 계속 유지한다.
- 이 값은 아직 메인 게임 공식 확정값이 아니라 prototype 기준값이다.

## 마우스 시야 이동 메모

- 마우스 시야 이동 기능을 추가했다.
- 이 기능은 조준/발사 기능이 아니라 1인칭 HUD 시야 조작감과 시야 이동 폭의 적절성을 확인하기 위한 것이다.
- Canvas를 클릭하면 Mouse View Mode가 활성화된다.
- Pointer Lock이 제한되는 환경에서는 Canvas를 드래그하는 동안 같은 제한 범위 안에서 시야 이동을 확인한다.
- `Esc` 키로 Mouse View Mode를 해제할 수 있다.
- 키보드 조작은 계속 사용할 수 있다.
- Earth Scale `6x`는 기본 기준값으로 유지한다.

## 조작 방법

- Arrow keys 또는 WASD: 제한된 시야 이동
- `5`: Earth Scale 5x
- `6`: Earth Scale 6x
- `7`: Earth Scale 7x
- `R`: 기본 정면 보기로 복귀. View Offset X/Y만 0으로 되돌리고, 사용자가 선택한 Earth Scale은 유지한다.
- 화면 버튼: Earth Scale 전환 및 기본 정면 보기 복귀. `Reset View`도 시야 위치만 초기화하고 Earth Scale은 유지한다.
- Canvas 클릭: Mouse View Mode 활성화
- 마우스 이동 또는 Canvas 드래그: 제한된 범위 안에서 시야 이동
- `Esc`: Mouse View Mode 해제
- Mouse Sensitivity: Low / Medium / High

## 현재 설정 패널

- 수치 표시 순서는 지구 구도값, 시야 오프셋, 달 표면 비중 순서로 정리한다.
- 첫 줄: Earth Scale / Earth Vertical Position
- 둘째 줄: View Offset X / View Offset Y
- 셋째 줄: Lunar Surface Area

## 확인할 질문

- Earth Scale 5x, 6x, 7x 중 어느 쪽이 더 게임 화면에 적절한가?
- Earth Vertical Position 30%가 1인칭 화면에서 자연스러운가?
- Lunar Surface Area 30%가 달 표면에 서 있다는 감각을 주는가?
- 제한된 시야 이동이 답답함보다 긴장감과 현장감을 주는가?

## 제외한 기능

미사일, 발사, 격추, 점수, 적, 사운드, 마우스 조준, 게임 승패 조건은 이번 범위에서 제외합니다.

또한 공격 위성, 타겟 락온, 카메라 반동, 메인 게임 구현, simulator 저장소 수정은 포함하지 않는다.

## 다음 단계 후보

- Earth Scale 5x/6x/7x 비교 결과 기록
- Earth Vertical Position과 Lunar Surface Area 후보값 조정 여부 판단
- 제한된 시야 이동 범위가 적절한지 확인
- 마우스 기반 시야 이동 감도와 범위가 적절한지 확인
- 지구-달 겹침 구도를 공격 위성 궤도선 또는 달 표면 이미지 추가 뒤 재검토
- 이후 별도 prototype에서 마우스 조준 또는 전투 감각을 독립적으로 실험
