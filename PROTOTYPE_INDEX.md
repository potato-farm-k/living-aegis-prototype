...
- Notes: `prototype-06-attack-source-trajectory`의 공격 원천 → 접근 경로 → Lunar Defense Zone 흐름을 바탕으로, Origin Type(Earth Surface / Orbital)과 Source Position(High / Low)을 구분하되 네 조합에 공통 world-space trajectory model, Lunar Defense Zone 기준 terminal entry 구간, 화면 기준 visibility 판정을 사용하는 프로토타입
