...
function nextThreat() {
  ...
  let nextIndex = state.threatIndex;

  while (nextIndex === state.threatIndex) {
    nextIndex = Math.floor(Math.random() * threatPatterns.length);
  }

  state.threatIndex = nextIndex;
  updateSettings(resolveThreat(state.viewportWidth, state.viewportHeight));
}
...
function drawThreat(threat, width, height, timestamp) {
  ...
  ctx.beginPath();
  ctx.fillStyle = "rgba(255, 107, 74, 0.92)";
  ctx.arc(0, 0, 4.5, 0, Math.PI * 2);
  ctx.fill();

  ctx.beginPath();
  ctx.strokeStyle = `rgba(255, 154, 82, ${0.42 + pulse * 0.28})`;
  ctx.lineWidth = 2;
