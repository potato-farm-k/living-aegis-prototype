...
`Final Approach Start`도 같은 preset 안에서 약 78% ~ 86% 사이로 선택한다. replay / reset 시 impact offset과 final approach start를 다시 선택하고, pause / resume은 선택값을 유지한다. visible defense anchor / Lunar Defense Zone marker는 계속 숨기며, debug panel에서 selected impact offset, final approach start, variation mode만 확인한다.
...
- selected impact offset / final approach start / variation mode debug text
...
- debug panel에는 hidden anchor 상태와 selected impact offset / final approach start / variation mode를 텍스트로 표시한다.
...
선택된 impact point는 내부 logical target으로만 사용하며, 화면에는 visible anchor / ring / label로 표시하지 않는다. 이 variation은 불공정한 무작위 공격이 아니라, 항상 같은 점으로 들어오는 느낌을 줄이기 위한 제한된 player-centered 범위다.
