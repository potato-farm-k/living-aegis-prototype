...
이번 defense anchor 보정에서는 `Lunar Defense Zone`을 앞쪽 지면에 놓인 target ring이 아니라, 플레이어가 위치한 생체 방어 거점 주변의 `player-centered defense anchor`로 해석한다. 기존 source / boost / main trajectory 구조는 유지하고, final approach와 impact feedback의 목표 anchor를 화면 하단 중앙의 near-field 방어권으로 보정했다. 따라서 위협은 마지막에 멀리 앞쪽 지면으로 떨어지는 것이 아니라, 플레이어 주변 방어권 / 생체 포대 상단으로 들어오는 흐름으로 읽히는지를 검토한다.
...
- player-centered defense anchor 기준의 Lunar Defense Zone 표시
- final approach와 Impact Point를 플레이어 중심 near-field 방어권으로 보정
...
- Impact Warning Corridor에 들어가면 threat pulse, corridor cue, Lunar Defense Zone ring, HUD warning의 강도가 함께 올라간다.
...
- 현재 기본값은 `Player Centered`이며, `Lunar Defense Zone`을 플레이어가 위치한 생체 방어 거점 주변의 최종 방어권으로 본다.
- main trajectory와 phase structure는 유지하고, final approach / impact 구간만 player-centered anchor로 수렴하게 했다.
- debug panel에는 `Defense Anchor: Player Centered`, `Impact Anchor: Player Centered`를 표시한다.
- `Forward Ground` 비교 모드는 이번 기본값으로 채택하지 않고, 필요할 때 후속 비교 후보로 남긴다.
...
9. Lunar Defense Zone이 앞쪽 지면 목표물이 아니라 player-centered defense anchor로 읽히는가?
10. Impact Point가 플레이어 방어권 / 생체 포대 상단 근처로 들어오는 느낌인가?

## 다음 단계 후보

- P0 Missile-type Threat의 실제 고도감 표현 검토
- Lunar Defense Zone 접근의 충돌 / 표면 연출 검토
- Forward Ground / Player Centered defense anchor 비교 필요성 검토
