...
이번 Earth backdrop 보정에서는 `Earth Visual Mode`, `Earth Visual Scale`, `Earth Position`을 control panel에서 바꿀 수 있게 했다. 기본값은 `telephoto-compressed` / `4x` / `upper-center`이며, 지구를 실제 천문 스케일이 아니라 망원 압축감에 가까운 거대한 정서적 배경으로 보았을 때 source / origin 감각과 화면 압박감이 좋아지는지 확인하기 위한 visual tuning option이다.

`Dramatic Earth Backdrop`과 `Telephoto-compressed Earth`는 Art Direction 후보를 prototype 안에서 감각 확인하기 위한 표현이다. 이번 단계에서 `living-aegis-origin`의 공식 Art Direction이나 GDD에 확정 반영하지 않으며, 실제 Earth-Moon scale physics나 천문 스케일 시뮬레이션도 구현하지 않는다.
...
- Earth Visual Mode: `realistic-ish` / `enlarged` / `telephoto-compressed` / `dramatic-backdrop`
- Earth Visual Scale: 실제 천문 스케일이 아닌 prototype 전용 visual scale `1x` / `2x` / `4x` / `6x` / `8x`
- Earth Position: `center` / `upper-center` / `side` / `low-horizon`
...
- Earth Visual Mode / Earth Visual Scale / Earth Position / Backdrop Compression tuning
...
또한 실제 미사일 탄도, 중력, 궤도 계산, 실제 천문 스케일 시뮬레이션, Earth-Moon scale physics 변경, 복잡한 orbital threat boost model, mouse-look 확장, off-screen arrow 정밀 보정, 요격체 궤적, 체력, 점수, 게임 오버, 여러 위협 동시 출현, 메인 게임 구현, origin / simulator 저장소 수정은 포함하지 않는다.
...
## Earth Backdrop Visual Tuning

이번 option은 큰 지구 배경이 LAO의 고독감, 압박감, 위협의 발원지 감각을 강화하는지 보기 위한 prototype-only tuning이다.

- `realistic-ish`: 비교적 작은 관측 대상에 가까운 지구
- `enlarged`: source 방향 가독성을 위해 키운 지구
- `telephoto-compressed`: 전경과 배경의 거리가 압축되어 지구가 더 크고 가까워 보이는 모드
- `dramatic-backdrop`: 현실성보다 감정선과 압박감을 우선한 거대한 지구 배경 모드

`Earth Visual Scale`은 실제 천문 배율이 아니라 canvas backdrop scale이다. `Backdrop Compression`은 별도 물리 projection이 아니라 mode preset 안의 시각적 압축감 표시다. PM 검토에서는 큰 지구가 threat marker, trail, Impact Warning Corridor, Lock Ready HUD를 방해하지 않으면서 source / origin 감각을 강화하는지 확인한다.
...
11. 큰 Earth backdrop이 고독감과 압박감을 강화하는가?
12. Earth Visual Mode / Scale이 커져도 threat marker와 HUD 가독성이 유지되는가?
