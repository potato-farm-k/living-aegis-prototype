...
- 새 위협 생성 시 source marker에서 짧은 launch pulse를 표시한다.
...
- 위협은 한 번에 1개만 등장한다.
- 위협은 공격 원천 근처에서 시작한다.
- 위협은 world/view-space 기준의 단순 곡선 경로를 따라 달 표면 위험 영역 쪽으로 이동한다.
...
점수, 체력, 게임 오버, 웨이브, 난이도 상승, 여러 위협 동시 출현, 사운드, 실제 총알/요격체 궤적, 실제 미사일 궤도, 실제 공격 위성 궤도 계산, 지구 표면의 정확한 발사 좌표 계산은 이번 범위에서 제외합니다.
