...
- 위협은 world/view-space 기준의 단순 직선 또는 완만한 곡선 경로를 따라 달 표면 근처 `Lunar Defense Zone` / 달 방어선 쪽으로 이동한다.
...
- 위협이 달 표면 위쪽 경계의 `Lunar Defense Zone` 가까이 도달하면 `Impact Warning`을 표시한다.
...
- 경로 끝이 향하는 `Lunar Defense Zone` 표시
