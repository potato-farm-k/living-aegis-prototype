...
이 값들은 최종 게임 기준으로 확정된 값이 아니라, 공격 원천과 위협 접근 경로를 확인하기 위한 prototype 기준값이다.
...
- 경로 끝이 향하는 하단 `Lunar Defense Zone` 표시
- 움직이는 위협 1개
...
점수, 체력, 게임 오버, 웨이브, 난이도 상승, 여러 위협 동시 출현, 사운드, 실제 총알/요격체 궤적, 실제 미사일 궤도, 실제 공격 위성 궤도 계산, 실제 공격무기 궤도, 지구 표면 발사, 지구 궤도 위성 발사, 지구 표면의 정확한 발사 좌표 계산, 종착지/충돌 판정은 이번 범위에서 제외합니다.
